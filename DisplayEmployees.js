...
function displayEmployee(users,user) {
    ...
    for (let i = 0; i < users.length; i++) {
        if(users[i].getId() == user.getId()){
            tbl.innerHTML += '<tr><td>' + users[i].getId() + '</td><td>' + users[i].getName() + '</td><td>' + users[i].getGender() + '</td><td>' + users[i].getDob() + '</td><td>' + users[i].getUsername() + '</td><td>'+users[i].getPosition()+'</td><td>'+users[i].getSalary()+' $</td><td><button type="button" onclick=" gotoEditEmployee(' + users[i].getId() + ')">Edit</button></td><td></td></tr>'
        }else{
