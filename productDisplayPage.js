...
function displayProduct(products) {
    ...
    if (user.getPosition() == 'Employee') {
        ...
        for (let i = 0; i < products.length; i++) {
            tbl.innerHTML += '<tr><td>' + products[i].getId() + '</td><td>' + products[i].getName() + '</td><td>' + products[i].getType() + '</td><td>' + products[i].spec + '</td><td>' + products[i].getPrice() + ' $</td><td><img src="' + products[i].getImgurl() + '" style="width: 150px; height: 70px"></td><td><button type="button" onclick="gotoEdit(' + products[i].getId() + ')">Edit</button></td><td><button type="button" onclick="gotoDelete(' + products[i].getId() + ')">Delete</button></td></tr>'
        }
    }
}
